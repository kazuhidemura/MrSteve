...
def _infer_question_mark(x, total_product):
    ...
    for i in range(len(x)):
        if i != question_mark_index:
            assert isinstance(x[i], int), f"when there is a question mark, there can be no other unknown values (full list: {x})"
...
def _ground(x, known, infer_question_mark_with=None):
    ...
    for part in x:
        assert isinstance(part, int), f"cannot infer value of {part}"
    return x
